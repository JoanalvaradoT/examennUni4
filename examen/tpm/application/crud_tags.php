<?php
session_start();
require_once '../../app/TagController.php';

$tagController = new TagController();
$tags = $tagController->obtenerTags();
  ...
  switch ($action) {
    case 'create':
      if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $result = $tagController->crearTag($data);

        if (isset($result['message']) && $result['message'] === 'Registro creado correctamente') {
          header("Location: /unidad4/examen/tpm/application/crud_tags.php?message=Etiqueta creada correctamente");
          exit();
        } else {
        }
      }
    ...
    case 'delete':
      if (isset($_GET['id'])) {
        $result = $tagController->eliminarTag($_GET['id']);

        if (isset($result['message']) && $result['message'] === 'Registro eliminado correctamente') {
          header("Location: /unidad4/examen/tpm/application/crud_tags.php?message=Etiqueta eliminada correctamente");
          exit();
        } else {
        }
      }
...
<body data-pc-preset="preset-1" data-pc-sidebar-theme="light" data-pc-sidebar-caption="true" data-pc-direction="ltr"
  ...
  <div class="pc-container">
    ...
    <div class="pc-content">
      <h1>CRUD de Etiquetas</h1>
      <?php if (isset($_GET['message'])): ?>
        <div class="alert alert-success"><?= htmlspecialchars($_GET['message']) ?></div>
      <?php endif; ?>

      <a href="/unidad4/examen/tpm/application/crear_tag.php" class="btn btn-primary">Crear Etiqueta</a>
      <div class="mt-4">
        <?php if ($tags && isset($tags['data'])): ?>
          <?php foreach ($tags['data'] as $tag): ?>
            <div class="card mb-3">
              <div class="card-body">
                <h5 class="card-title"><?= htmlspecialchars($tag['name']) ?></h5>
                <p class="card-text"><?= htmlspecialchars($tag['description']) ?></p>
                <a href="?action=delete&id=<?= $tag['id'] ?>" class="btn btn-danger"
                  onclick="return confirm('¿Estás seguro de eliminar esta etiqueta?');">Eliminar</a>
              </div>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
