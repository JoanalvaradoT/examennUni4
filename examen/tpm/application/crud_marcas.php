<?php
session_start();
require_once '../../app/MarcaController.php';
  ...
  switch ($action) {
    case 'create':
      if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $result = $marcaController->crearMarca($data);

        if (isset($result['message']) && $result['message'] === 'Registro creado correctamente') {
          header("Location: /unidad4/examen/tpm/application/crud_marcas.php?message=Marca creada correctamente");
          exit();
        } else {
          echo "<p>Error al crear la marca: " . print_r($result, true) . "</p>";
        }
      }
      break;
    ...
    case 'delete':
      if (isset($_GET['id'])) {
        $result = $marcaController->eliminarMarca($_GET['id']);

        if (isset($result['message']) && $result['message'] === 'Registro eliminado correctamente') {
          header("Location: /unidad4/examen/tpm/application/crud_marcas.php?message=Marca eliminada correctamente");
          exit();
        } else {
          echo "<p>Error al eliminar la marca: " . print_r($result, true) . "</p>";
        }
      }
      break;
...
<body data-pc-preset="preset-1" data-pc-sidebar-theme="light" data-pc-sidebar-caption="true" data-pc-direction="ltr"
  ...
  <div class="pc-container">
    ...
    <div class="pc-content">
      <h1>CRUD de Marcas</h1>
      <?php if (isset($_GET['message'])): ?>
        <div class="alert alert-success"><?= htmlspecialchars($_GET['message']) ?></div>
      <?php endif; ?>

      <a href="/unidad4/examen/tpm/application/crear_marca.php" class="btn btn-primary">Crear Marca</a>
      <div class="mt-4">
        <?php if ($marcas && isset($marcas['data'])): ?>
          <?php foreach ($marcas['data'] as $marca): ?>
            <div class="card mb-3">
              <div class="card-body">
                <h5 class="card-title"><?= htmlspecialchars($marca['name']) ?></h5>
                <p class="card-text"><?= htmlspecialchars($marca['description']) ?></p>
                <a href="?action=delete&id=<?= $marca['id'] ?>" class="btn btn-danger"
